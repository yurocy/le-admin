...
async function fetchCategories() {
  try { const res: any = await pricingApi.listCategory(); categoryList.value = res.data || res || [] } catch { /* ignore */ }
}
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) { await pricingApi.updateBrand(editId.value, { ...form }); ElMessage.success('更新成功') }
    else { await pricingApi.createBrand({ ...form }); ElMessage.success('添加成功') }
    dialogVisible.value = false; fetchData()
  } catch { ElMessage.error('操作失败') } finally { submitLoading.value = false }
}

async function handleDelete(row: any) {
  try { await ElMessageBox.confirm('确定删除该品牌吗？', '提示', { type: 'warning' }); await pricingApi.deleteBrand(row.id); ElMessage.success('删除成功'); fetchData() } catch { /* cancelled */ }
}
